...
struct _ATL_OBJMAP_ENTRY30
{
		...
		_In_ DWORD dwFlags)
	{
		...
		if (pfnGetClassObject == NULL)
			return S_OK;
		HRESULT hRes = pfnGetClassObject(reinterpret_cast<void*>(pfnCreateInstance), __uuidof(IUnknown), (LPVOID*) &p);
		if (SUCCEEDED(hRes))
		...
		while (pEntry->pclsid != NULL)
		{
			if ((pEntry->pfnGetClassObject != NULL) && InlineIsEqualGUID(rclsid, *pEntry->pclsid))
			{
				if (pEntry->pCF == NULL)
				{
					...
					if (pEntry->pCF == NULL)
					{
						hr = pEntry->pfnGetClassObject(reinterpret_cast<void*>(pEntry->pfnCreateInstance), __uuidof(IUnknown), (LPVOID*)&pEntry->pCF);
					}
				}
			...
			if ((pEntry->pfnGetClassObject != NULL) && InlineIsEqualGUID(rclsid, *pEntry->pclsid))
			{
				...
				if (pCache->pCF == NULL)
				{
					...
					if (pCache->pCF == NULL)
					{
						...
						hr = pEntry->pfnGetClassObject(reinterpret_cast<void*>(pEntry->pfnCreateInstance), __uuidof(IUnknown), reinterpret_cast<void**>(&factory));
						if (SUCCEEDED(hr))
						{
