...
{
    static bool isUnion(const UdtInfo& u) { return u.udtKind == UdtUnion; }
    ...
    static std::wstring BstrToWstr(BSTR b) { return b ? std::wstring(b) : std::wstring(L"<unnamed>"); }
    ...
    static void printUdt(const UdtInfo& u)
    {
        ...
        std::wcout << L"    kind=" << OdrCop::udtKindName(u.udtKind) << L"  size=" << u.size << L'\n';
   ...
   ~OdrCop() { CoUninitialize(); }
                            ...
                            {   // Skip anonymous/unnamed UDTs
                                ...
                                if (SUCCEEDED(udt->get_name(&name)) && name && name[0] != L'\0') {
                                    ...
                                    if (key.find(L'<') == std::wstring::npos && key.find(L"lambda") == std::wstring::npos)
                                        (*this)[key].push_back(materializeUdt(udt, path));
                                }
                                udt.Release();
                            }
                        }
    ...
    int ReportViolations() const
    {
        ...
        for (auto& [name, definitions] : *this)
        {
            ...
            {
                if (!OdrCop::udtEqual(definitions[0], definitions[i]))
                {
                    allSame = false;
                    break;
                }
            ...
            for (size_t i=0; i<definitions.size(); ++i)
            {
                ...
                {
                    if (!printed[j] && OdrCop::udtEqual(definitions[i], definitions[j]))
                    {
